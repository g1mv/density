...
pub trait Codec: QuadEncoder + QuadDecoder {
    ...
    #[inline(always)]
    fn encode_block(&mut self, block: &[u8], out_buffer: &mut EncodeBuffer, signature: &mut EncodeSignature, protection_state: &mut ProtectionState) {
    ...
    fn encode(&mut self, input: &[u8], output: &mut [u8]) -> Result<usize, EncodeError> {
        let mut signature = EncodeSignature::new(0);
    ...
    fn decode(&mut self, input: &[u8], output: &mut [u8]) -> Result<usize, DecodeError> {
        let mut in_buffer = DecodeBuffer::new(input);
        let mut out_buffer = EncodeBuffer::new(output, 0);
        let mut protection_state = ProtectionState::new();

        while in_buffer.remaining() >= BYTE_SIZE_U64 + self.block_size() {
            if protection_state.revert_to_copy() {
                out_buffer.push(in_buffer.read(self.block_size()));
                protection_state.decay();
            } else {
                let mark = in_buffer.index;
                let mut signature = DecodeSignature::new(in_buffer.read_u64());
                for _ in 0..self.block_size() >> 3 {
                    let quads = self.decode_quads(&mut in_buffer, &mut signature);
                    out_buffer.push(&quads.0.to_le_bytes());
                    out_buffer.push(&quads.1.to_le_bytes());
                }
